...
var endMessage = function () {
    ...
    d3.select("#instructions")
        .append("p")
        .html("Wow good job you reach  the end!");
};
var displayInstructions = function () {
    ...
    d3.select("#instructions")
        .append("p")
        .html(
            "1. Spot it as fast as possible and press <code>Space</code> bar;"
        );
    ...
    if (visualVariable == "Color") {
        ...
        for (var i = 2 * (objectCount / 3); i < objectCount - 1; i++) {
            ...
            if (targetShadow == "shadowOff" && targetColor == "LightGray") {
                ...
                targetShadow == "shadowOff" &&
                targetColor == "DarkGray"
            ) {
                objShadow = "shadowOn";
                objColor = "LightGray";
            } else if (
                targetShadow == "shadowOn" &&
                targetColor == "DarkGray"
            ) {
                objShadow = "shadowOff";
                objColor = "LightGray";
...
var displayPlaceholders = function () {
    ...
    for (var i = 0; i < objectCount; i++) {
        ...
        placeholder.on("click", function () {
            ...
            if (ctx.targetIndex !== counter) errorCounter++;
            else {
                errorCounter = 0;
            }

            if (
                ctx.cpt + 1 >= ctx.trials.length ||
                ctx.trials[ctx.cpt + 1][ctx.participantIndex] != ctx.participant
            ) {
                endMessage();
            } else {
                nextTrial();
